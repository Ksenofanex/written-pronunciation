...
class WordUpdateView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def test_func(
        self,
    ):  # For authorization. If logged in user is not the author itself, it can't edit & delete.
...
class WordDeleteView(LoginRequiredMixin, UserPassesTestMixin, DeleteView):
    ...
    success_url = reverse_lazy(
        "home"
    )  # After action is successfull, redirect to the desired page.
...
class SearchResultsView(ListView):
    ...
    def get_queryset(self):
        ...
        )  # Searching for field. In this case, English field.
        return object_list
